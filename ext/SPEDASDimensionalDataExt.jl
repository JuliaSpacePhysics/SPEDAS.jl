...
using DimensionalData: AbstractDimArray, DimArray, DimSelectors, Near, Dim
import SPEDAS
...
function SPEDAS.rotate(da::AbstractDimArray, mats::AbstractVector)
    ...
    DimArray(data, DimensionalData.dims(da); name=da.name, metadata=da.metadata)
end
...
function SPEDAS._rebuild_data(x::AbstractDimArray, data)
    DimensionalData.rebuild(x; data)
end
...
function SPEDAS._update_coord_dims(new_da::AbstractDimArray, da::AbstractDimArray, old_coords, old_new_pairs)
    ...
    if any(old -> occursin(old, name_str), old_coords)
        ...
        new_da = DimensionalData.rebuild(new_da; name = Symbol(new_name))
    end
